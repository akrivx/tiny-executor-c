...
static inline const texec_queue_create_full_policy_info_t* find_queue_full_policy_info(const texec_queue_create_info_t* info) {
...
static inline bool queue_full_reject(const texec_queue_t* q) {
  return q->full_policy == TEXEC_QUEUE_FULL_REJECT;
}

static inline texec_status_t queue_push_impl(texec_queue_t* q, uintptr_t item, bool just_try) {
  ...
  while (!q->closed && queue_is_full(q)) {
    if (just_try) {
      return queue_unlock_return(q, TEXEC_STATUS_NOT_READY);
    }
    if (queue_full_reject(q)) {
...
static inline texec_status_t queue_pop_impl(texec_queue_t* q, uintptr_t* out_item, bool just_try) {
